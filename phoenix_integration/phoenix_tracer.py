...
def wrap_client(client: Any, provider: str = "auto") -> Any:
    ...
    try:
        ...
        logger.error(f"Failed to wrap {provider} client: {e}")
        return client
...
def log_ocr_result(
    ...
    try:
        with px.span(name=f"{agent_name}.ocr_result") as span:
...
def log_route_decision(
    ...
    try:
        with px.span(name="orchestrator.route_decision") as span:
...
def log_knowledge_retrieval(
    ...
    try:
        with px.span(name="knowledge_base.retrieval") as span:
            span.set_attribute("query", query[:500])
