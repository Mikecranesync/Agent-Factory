# HARVEST BLOCK 7: Database Manager (Multi-Provider PostgreSQL)

**Priority**: CRITICAL - Foundation Layer
**Size**: 29KB (832 lines)
**Source**: `agent_factory/core/database_manager.py`
...
Multi-provider database manager with automatic failover across PostgreSQL providers (Supabase, Railway, Neon, Atlas, VPS, Local SQLite).

### What This Adds

- **Multi-Provider Support**: 6 providers (Supabase, Railway, Neon, Atlas, VPS, Local SQLite)
- **Automatic Failover**: Seamless failover on connection/query errors
- **Connection Pooling**: psycopg pools per provider (2-20 connections)
- **Health Checks**: Cached health status (60s TTL) to avoid overhead
- **Async Support**: Both sync and async query execution
- **Local Fallback**: SQLite as ultimate fallback when all cloud providers fail
...
result = db.execute_query("SELECT * FROM knowledge_atoms LIMIT 10")
# Tries: neon → supabase → railway → local SQLite
...
# {"neon": True, "supabase": True, "railway": False, "local": True}

# Async execution
atoms = await db.fetch_all_async("SELECT * FROM knowledge_atoms WHERE vendor = $1", "siemens")
```
...
DATABASE_PROVIDER=neon  # Options: neon, supabase, railway, atlas, vps, local
DATABASE_FAILOVER_ENABLED=true
DATABASE_FAILOVER_ORDER=neon,supabase,railway,local

# Provider credentials
NEON_DB_URL=postgresql://user:password@ep-xxx.neon.tech/rivet_db?sslmode=require
SUPABASE_DB_PASSWORD=your_password_here
SUPABASE_DB_HOST=db.xxx.supabase.co
RAILWAY_DB_URL=postgresql://user:password@containers.railway.app:5432/railway
ATLAS_DB_URL=postgresql://postgres:password@localhost:5432/atlas
VPS_KB_HOST=72.60.175.144
VPS_KB_PORT=5432
VPS_KB_USER=rivet
VPS_KB_PASSWORD=rivet_factory_2025!
VPS_KB_DATABASE=rivet
LOCAL_DB_PATH=data/local.db  # SQLite fallback
```
...
```bash
poetry add 'psycopg[binary]'  # PostgreSQL driver with binary optimizations
poetry add psycopg-pool        # Connection pooling
poetry add asyncpg             # Async PostgreSQL (for async_query methods)
poetry add aiosqlite           # Async SQLite (for local fallback)
```
...
### File: `rivet/core/database_manager.py`
...
result = supabase.table("atoms").select("*").limit(10).execute()
```

**After** (DatabaseManager with failover):
```python
from rivet.core.database_manager import DatabaseManager

db = DatabaseManager()
result = db.execute_query("SELECT * FROM atoms LIMIT 10")
```

### 2. Update Memory Storage

Modify `rivet/memory/storage.py` to use DatabaseManager:
...
### 3. Add Failover Logging

Monitor which provider handles each request:

```python
# DatabaseManager logs automatically:
# INFO: Executing query on neon
# WARNING: Executed on failover provider 'supabase' (primary 'neon' unavailable)
```

### 4. Configure Fallback Order

Customize failover sequence based on reliability:

```bash
# In .env
DATABASE_FAILOVER_ORDER=neon,supabase,railway,vps,atlas,local
```
...
python -c "from rivet.core.database_manager import DatabaseManager; db = DatabaseManager(); print('Providers:', list(db.providers.keys()))"

# Test health checks
python -c "from rivet.core.database_manager import DatabaseManager; db = DatabaseManager(); print(db.health_check_all())"

# Test query execution
python -c "from rivet.core.database_manager import DatabaseManager; db = DatabaseManager(); print(db.execute_query('SELECT version()', fetch_mode='one'))"

# Test failover (disable primary in .env, then run query)
python -c "from rivet.core.database_manager import DatabaseManager; db = DatabaseManager(); db.set_provider('supabase'); print(db.execute_query('SELECT 1', fetch_mode='one'))"
```
...
After implementing HARVEST 7, you will have:

- ✅ **Resilient Database Access** - Automatic failover across 6 providers
- ✅ **Zero Downtime** - Seamless failover on connection/query errors
- ✅ **Local Development** - SQLite fallback works without cloud credentials
- ✅ **Production Ready** - Connection pooling + health monitoring
- ✅ **Async Support** - Both sync and async query execution
- ✅ **Unified Interface** - Single API across all providers

This foundation enables all other HARVEST BLOCKS to work with any database provider without modification.

---

## Next Steps

After implementing HARVEST 7:
1. ✅ Test with all configured providers
2. ✅ Verify failover works (disable primary, run queries)
3. ✅ Proceed to **HARVEST 8** (PostgreSQL Memory Storage)
4. Update existing code to use DatabaseManager instead of direct connections
